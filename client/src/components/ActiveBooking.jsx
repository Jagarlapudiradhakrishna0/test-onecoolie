...
export default function ActiveBooking({ booking, onUpdate, distance = 500 }) {
  ...
  const submitRating = async () => {
    ...
    // Save to localStorage immediately so it NEVER asks again
    localStorage.setItem(`rated_${booking.id}`, 'true');
  ...
  return (
    <div className="space-y-4 sm:space-y-6 w-full max-w-full overflow-hidden">
              ...
              return (
                <div key={step.key} className="space-y-1.5 sm:space-y-2 relative text-center sm:text-left min-w-0">
                  <div className="flex items-center gap-1 sm:gap-2 justify-center sm:justify-start">
                    <span className={`w-5 h-5 sm:w-6 sm:h-6 rounded-full flex items-center justify-center text-[10px] sm:text-xs font-bold shrink-0 transition-all ${isPassed
                    ...
                    <div className="hidden sm:block flex-1 h-1 rounded-full bg-slate-200 overflow-hidden">
                      <div className={`h-full transition-all ${isPassed ? 'bg-emerald-500 w-full' : isCurrent ? 'bg-blue-600 w-1/2 animate-pulse' : 'w-0'
                        }`} />
                    </div>
                  </div>

                  <div className="px-0.5">
                    <p className={`text-[10px] sm:text-xs font-bold truncate ${isReached ? 'text-zinc-900' : 'text-zinc-400'
                      }`}>
                      {step.label}
                    </p>
                    <p className={`font-mono text-[9px] sm:text-[10px] font-semibold truncate ${isReached ? 'text-zinc-600' : 'text-zinc-300'
                      }`}>
                      {time}
                    </p>
                    <p className={`text-[9px] sm:text-[10px] hidden sm:block truncate ${isReached ? 'text-zinc-500' : 'text-zinc-400'
                      }`}>
                      {sub}
                    </p>
                  </div>
      ...
      <div className="grid lg:grid-cols-12 gap-4 sm:gap-8 items-start w-full">
          ...
          {!isCancelled && (
            <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-4 sm:p-8 space-y-4 w-full">
              <div className="flex items-center justify-between border-b border-slate-100 pb-3 sm:pb-4">
                ...
                <span className={`px-2.5 py-0.5 sm:px-3 sm:py-1 text-[10px] sm:text-[11px] font-semibold rounded-full border ${isCompleted ? 'bg-slate-100 text-zinc-500 border-slate-200' : 'bg-blue-50 text-blue-700 border-blue-200'
                  }`}>
                ...
                {chatMsgs.length === 0 ? (
                  ...
                  chatMsgs.map((m, i) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`max-w-xs px-4 py-2.5 rounded-2xl text-xs font-medium ${isPassenger
              ...
              <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/70 p-4 sm:p-7 shadow-[0_8px_30px_rgb(0,0,0,0.04)] space-y-3 w-full">
                ...
                <div className="flex items-center gap-1.5 pt-1">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <Star
                      key={star}
                      className={`w-5 h-5 ${star <= (booking.rating || rating || 5)
                ...
                {(booking.review || review) && (
                  <p className="text-xs text-zinc-700 bg-slate-50 p-3.5 rounded-xl border border-slate-200/60 font-medium leading-relaxed italic">
                    "{booking.review || review}"
                  </p>
                )}
              </div>
            ) : (
              <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/70 p-4 sm:p-8 shadow-xs space-y-4 w-full">
                ...
                <div className="flex items-center gap-2">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      ...
                      className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl border text-sm sm:text-base font-bold transition-all cursor-pointer ${rating >= star
                        ? 'bg-blue-600 text-white border-blue-600 shadow-md'
                        : 'bg-slate-50 text-zinc-400 border-slate-200 hover:border-slate-400'
                        }`}
                    >
                      ★
                    </button>
                ...
                <textarea
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl sm:rounded-2xl p-3.5 sm:p-4 text-xs text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-blue-600/30 focus:border-blue-600 transition-all h-24 resize-none"
                />
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <button
                    ...
                    className="bg-black hover:bg-zinc-800 disabled:opacity-50 text-white font-bold px-6 py-3 rounded-full text-xs transition-all cursor-pointer shadow-xs"
                  >
                  ...
                  <button
                    ...
                    className="px-5 py-3 rounded-full bg-slate-100 hover:bg-slate-200 text-zinc-700 font-bold text-xs transition-colors cursor-pointer"
                  >
                    Skip to Dashboard →
                  </button>
                </div>
        ...
        {/* Right Column (4 cols): Trip Summary, Service & Payment Ledger */}
        <aside className="lg:col-span-4 space-y-4 sm:space-y-6 w-full">
          {/* 1. Trip Summary Card */}
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-4 sm:p-6 space-y-4 sm:space-y-5 w-full">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <h3 className="text-base sm:text-lg font-extrabold tracking-tight text-zinc-900">
                Trip Summary
              </h3>
              <span className={`px-2 py-0.5 sm:px-2.5 sm:py-0.5 text-[9px] sm:text-[10px] font-extrabold rounded-full flex items-center gap-1.5 uppercase ${tripSummaryBadge.bg}`}>
                {tripSummaryBadge.pulse && <span className="w-1.5 h-1.5 rounded-full bg-blue-600 animate-pulse" />}
                {tripSummaryBadge.icon && <span>{tripSummaryBadge.icon}</span>}
            ...
            <div className="space-y-3.5 text-xs">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5 sm:gap-3 text-zinc-500 font-medium min-w-0">
                  <span className="w-7 h-7 sm:w-8 sm:h-8 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center shrink-0 border border-blue-100">
                    <Train className="w-3.5 h-3.5 sm:w-4 sm:h-4" />
                  </span>
                  <div className="min-w-0">
                    <p className="font-black text-xs sm:text-sm text-zinc-900 truncate">
                      {booking.train_no || booking.train_number || '12615'}
                    </p>
                    <p className="text-[9px] sm:text-[10px] text-zinc-400">Train Number</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5 sm:gap-3 text-zinc-500 font-medium min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[9px] sm:text-[10px] text-zinc-400">Train Name</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5 sm:gap-3 text-zinc-500 font-medium min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[9px] sm:text-[10px] text-zinc-400">Station Hub</p>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2.5 sm:gap-3 text-zinc-500 font-medium min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[9px] sm:text-[10px] text-zinc-400">Journey Date</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* 2. Service & Payment Card */}
          <div className="bg-white rounded-2xl sm:rounded-3xl border border-slate-200/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-4 sm:p-6 space-y-4 w-full">
            <h3 className="text-base sm:text-lg font-extrabold tracking-tight text-zinc-900 border-b border-slate-100 pb-3">
              Service &amp; Payment
            </h3>

            <div className="space-y-2.5 sm:space-y-3 text-xs">
              <div className="flex justify-between items-center text-zinc-500">
                <span>Coach Escort</span>
                <span className="font-bold text-zinc-900">Yes</span>
              </div>
              <div className="flex justify-between items-center text-zinc-500">
                <span>Luggage Assistance</span>
                <span className="font-bold text-zinc-900">
                  {booking.services?.luggage ? `${booking.services.luggage} item(s)` : '1 item(s)'}
                </span>
              </div>
              <div className="flex justify-between items-center text-zinc-500">
                <span>Seat/Berth Support</span>
                <span className="font-bold text-zinc-900">Yes</span>
              </div>
              <div className="flex justify-between items-center text-zinc-500">
            ...
            <div className="border-t border-slate-100 pt-3.5 sm:pt-4 flex justify-between items-center">
              <span className="text-xs sm:text-sm font-extrabold text-zinc-900">Total Amount</span>
              <span className="text-xl sm:text-2xl font-black font-mono text-zinc-900">
                ₹{booking.total_price || 90}
              </span>
            </div>
            ...
            {!isCompleted && !isCancelled && status !== 'in_service' && (
              <button
                ...
                className="w-full py-2.5 sm:py-3 rounded-xl sm:rounded-2xl bg-rose-50 hover:bg-rose-100 text-rose-700 font-extrabold text-xs transition-colors cursor-pointer flex items-center justify-center gap-2 border border-rose-200/80 shadow-2xs mt-2"
              >
      ...
      <ConfirmDialog
        ...
        confirmText="Transmit SOS Alert"
        cancelText="Dismiss"
